...
static DefVungleListener     defUtoLua;
static dmArray<CallbackData>   m_callbacksQueue;
static dmMutex::HMutex         m_mutex;

static void RegisterCallback(lua_State* L, int index, DefVungleListener* cbk)
{
...
static void UnregisterCallback(DefVungleListener* cbk)
{
    if(cbk->m_Callback != LUA_NOREF)
    {
        dmScript::Unref(cbk->m_L, LUA_REGISTRYINDEX, cbk->m_Callback);
...
static void InvokeCallback(int type, char*key_1, char*value_1, char*key_2, char*value_2, DefVungleListener* cbk)
{
    if(cbk->m_Callback == LUA_NOREF)
    {
        dmLogInfo("DefVungle callback does not exist.");
        return;
    }
    ...
    if (!dmScript::IsInstanceValid(L)) {
        ...
        dmLogError("Could not run DefVungle callback because the instance has been deleted.");
        lua_pop(L, 2);
    }
    else {
